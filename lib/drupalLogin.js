...
module.exports = {
    ...
    login: function(room, body, event) {
        ...
        let alias, platform, state = room.currentState.getStateEvents(container.config.stateName, 'alias'),
        ...
        switch (platform) {
            case 'wp':
                ...
                let fullAlias = '@'+alias+'-'+env;
                let msg = 'Running <b>wp '+fullAlias+' login create '+ user+'</b>.';
                this.container.send(room, msg);

                let args = [fullAlias, 'login', 'create', user];
                // now execute wp
            ...
            default:
                let fullAlias = '@'+alias+'.'+env;
                let msg = 'Running <b>drush '+fullAlias+' user-login '+ user+'</b>.';
                this.container.send(room, msg);

                let args = [fullAlias, 'uli', '--browser=0'];
                if (user) {
                    args.push(user);
                }
                // Now execute drush...
