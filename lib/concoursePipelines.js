    ...
    setAction: function(room, body, event) {
        ...
        switch (args[1]) {
            case 'list': // queue list
                var message = '', template = "<li>{jobId}: {alias} / {job}</li>";
            ...
            case 'update':
                ...
                break;
            case 'wraith':
                this.queueJob(alias, 'run-wraith', 0);
                break;
            case 'export':
                switch (args[2]) {
                    case 'stage':
            ...
            case 'bump':
                if (args[0] == 'queue') {
                    // TODO: Implement queue bump
                    this.container.send(room, 'Queue bump not yet implemented.');
                } else {
                    this.queueJob(alias, 'patch', 'patch', 0);
                }
                break;
            case 'kill':
                // TODO: Implement queue kill...
                this.container.send(room, 'Queue kill not yet implemented.');
                break;
            ...
            case 'test':
            case 'behat':
                this.queueJob(alias, 'run-behat', 'run-behat', 0);
                break;
            case 'approve':
                var stateName = 'test_' + arg[2];
    ...
    activatePipeline: function(topic, data, minutes, cb) {
        ...
        if (room = this.container.roomsByAlias[data.alias]) {
            if (data.branch == 'develop') {
                ...
                this.queueJob(data.alias, 'dev-update', 'run-behat');
            } else if (data.branch == 'release') {
                // trigger stage-update job
                this.queueJob(data.alias, 'deploy-code-stage', 'run-wraith');
