...
module.exports = {
    ...
    setup: function(container) {
        ...
        container.bangCommands['deploy'] = {
            ...
            args: ['prod', 'check', 'release', 'send summary'],
            cb: this.setAction.bind(this)
        };
    ...
    setAction: function(room, body, event) {
        ...
        switch (args[1]) {
            case 'list': // queue list
                let message = '', template = "<li>{jobId}: {alias} / {job} - end: {finish}</li>";
    ...
    startNextJob: function() {
        if (this.jobQueue.length && Sugar.Object.keys(this.currentJob).length < this.maxJobs) {
            let item = this.jobQueue.shift(), skip = [];
    ...
    */
    concourseMessage: function(room, body, event) {
                    ...
                    Sugar.Object.forEach(data, function (value, key) {
                        switch (key) {
                            ...
                            case 'version':
                                ...
                                if (job === 'deploy-code-prod' && trigger != 'fail') {
                    ...
                    if (this.cleanRun) {
                        ...
                        switch (state) {
                            ...
                            case 'stage':
                                ...
                                if (releaseDate) {
                                    this.sitesStage.push(alias + ' - ' + releaseNote + ' (release at: ' + releaseDate + ')');
                                } else {
                                    this.sitesStage.push(alias + ' - ' + releaseNote);
                                }
                                break;
                            case 'dev':
     ...
     console.log('check clean check:',alias,this.lastCheck, this.lastCheck === alias);
                }
    ...
    sendSummary: function(alias) {
        ...
        msg = '<h3>Check run completed for {count} sites.<h3>\n' +
            ...
            '<font color="blue"><h4>Beta sites: {betaCount}</h4>\n' +
            '<ul><li>{betaSites}</li></ul></font>\n' +
            '<font color="green"><h4>Dev sites: {devCount}</h4>\n' +
