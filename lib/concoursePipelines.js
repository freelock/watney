...
module.exports = {
    ...
    setup: function(container) {
        ...
        schedule.scheduleJob('16 0 * * *', this.checkClean.bind(this));
    ...
    setAction: function(room, body, event) {
        ...
        switch (args[1]) {
            ...
            case 'update':
                ...
                break;
            case 'wraith':
                this.queueJob(alias, 'run-wraith', 'extract-results', 0);
                break;
            case 'export':
                switch (args[2]) {
                    case 'stage':
            ...
            case 'test':
            case 'behat':
                this.queueJob(alias, 'run-behat', 'extract-results', 0);
                break;
            case 'approve':
    ...
    activatePipeline: function(topic, data, minutes, cb) {
        ...
        if (room = this.container.roomsByAlias[data.alias]) {
            if (data.branch == 'develop') {
                ...
                this.queueJob(data.alias, 'dev-update', 'extract-results');
            } else if (data.branch == 'release') {
                // trigger stage-update job
                this.queueJob(data.alias, 'deploy-code-stage', 'extract-results');
                    ...
                    if (this.cleanRun) {
                        ...
                        switch (state) {
                            case 'released':
                                ...
                                this.currentJob[alias].finish = 'sanitize-stage';
                                // fallthrough
                            case 'clean':
                                if (!cleanerrors) this.sitesClean.push(alias);
                                break;
                            case 'stage':
...
//                    setTimeout(this.startNextJob.bind(this),1);
     ...
     */
    checkClean: function() {
        var container = this.container, maint, alias, lastJob, self = this;
    ...
    sendSummary: function(alias) {
        ...
        data = {
            ...
            cleanCount: this.sitesClean.count(),
            failCount: this.sitesFail.count(),
            devCount: this.sitesDev.count(),
            stageCount: this.sitesStage.count(),
