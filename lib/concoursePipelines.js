        ...
        switch (args[1]) {
                    ...
                    if (container.getState(room, 'test_behat') === "1") {
                        if (container.getState(room, 'test_wraith') === "1") {
                            if (container.getState(room, 'state') === 'stage') {
                                ...
                                this.container.send(room, 'Send release message now - !notelify scheduled', 'greenMessage');
                            } else {
            ...
            case 'approve':
                ...
                if ((stateName === 'test_behat') || (stateName === 'test_wraith')) {
                    ...
                    // check after 4 seconds ... otherwise does not pick up state change.
                    setTimeout(() => { 
                        container.PubSub.publish('CheckSchedule', {room: room});
                    }, 4000);
                }
                break;
                // Dev commands
                    ...
                    Sugar.Object.forEach(data, function (value, key) {
                        switch (key) {
                            ...
                            case 'state':
                                ...
                                // Clear test status
                                if (value == 'clean') {
                                    container.setState(room, 'test_wraith', '0');
                                    container.setState(room, 'test_behat', '0');
                                }
                                if (value == 'stage') {
                                    container.setState(room, 'test_wraith', '0');
                                }
                            ...
                            case 'test_behat':
                                ...
                                container.setState(room, container.config.stateName, value, key);
                                if (value == 1) {
                                    container.PubSub.publish('CheckSchedule', data);
                                }
                                break;
                            case 'log':
                                log = value;
                            ...
                            case 'prod_version':
                                if (data.state == 'released') {
                                    //container.send(room, 'Send released message now - !notelify released', 'greenMessage');
                                    data.template = 'protection-released.mjml';
                                    data.subject = '{{ Website }} - Release deployed to production.';
                                    container.PubSub.publish('notelify', data);
                                }
                                container.setState(room, container.config.stateName, value, key);
