    ...
    setAction: function(room, body, event) {
        ...
        switch (args[1]) {
            ...
            case 'export':
                switch (args[2]) {
                    case 'stage':
                        this.triggerJob(room, body, alias, 'export-db-stage');
                        break;
                    case 'prod':
                        this.triggerJob(room, body, alias, 'export-db-prod');
                        break;
                    default:
            ...
            case 'prod':
                ...
                var last_check = container.getState(room, 'last_check_time');
                if (last_check) {
                    Sugar.Date.addHours(last_check, 24);
                }
                if (args[2] == 'force' && container.isReleaseManager(room, event)) {
                    this.triggerJob(room, body, alias, 'deploy-code-prod');
                } else if (container.getState(room, 'clean_check') == 'OK' && last_check && Sugar.Date.isFuture(last_check)) {
                    this.triggerJob(room, body, alias, 'deploy-code-prod');
                } else {
                ...
                break;
            case 'check':
                this.triggerJob(room, body, alias, 'check-clean');
                break;
      ...
      //  } else {
            ...
            if (room = container.roomsByAlias[alias]) {
                ...
                if (job == 'check-clean') {
                    ...
                    if (cleancheck == 'OK') {
                        if (container.getState(room, 'clean_check') != 'OK') {
                            message = alias + ' site is now clean.';
                            type = 'greenMessage';
                        }
                    } else {
                    ...
                    container.setState(room, container.config.stateName, new Date(), 'last_check_time');
                }
