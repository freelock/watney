    ...
    setAction: function(room, body, event) {
        ...
        switch (args[1]) {
            ...
            case 'hotfix':
                ...
                if (args[0] === '!queue') {
                    if (!args[2]) {
                        message = Object.keys(this.hotfixQueue)
                          .reduce((message, platform) => message += `Hotfixes for ${platform}:<ul><li>`
                            + this.hotfixQueue[platform].join('</li><li>')
                            + '</li></ul>',
                           '');
                        container.send(room, message);
                    } else {
                        message = args[2] + ' Hotfixes still applying to: <ul><li>' + this.hotfixQueue[args[2]].join('</li><li>') + '</li></ul>' 
                        container.send(room, message);
                    }
    ...
    */
            ...
            if (room = container.roomsByAlias[alias]) {
                ...
                if (job === 'check-clean') {
                    ...
                    if (this.cleanRun) {
                        ...
                        this.checkSites.filter((x)=> x != alias);
    ...
    getPlatformSites: function(platform) {
        ...
        rooms = container.roomList.filter(room => {
            version = container.getState(room, 'version');
            maint = container.getState(room, 'maintenance');
            if (maint === 'yes') {
                switch (platform) {
                    case version:
