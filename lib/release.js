...
module.exports = {
    ...
    setup: function(container) {
        ...
        container.PubSub.subscribe('CheckSchedule',
            this.checkSchedule.bind(this)
        );
        container.PubSub.subscribe('ReleaseScheduled',
            this.checkSchedule.bind(this)
        );
        ...
        container.getReleaseNote = this.getReleaseNote.bind(this);
        container.getPolicy = this.getPolicy.bind(this);
    ...
    releaseNotes: function(room, body, event) {
        ...
        var props = container.getState(room, '', container.config.releaseName);
        if (props) {
            active = props.active;
        ...
        if (!args[1]) {
            let cases = Object.keys(props.cases).map(item => this.printCase(props.cases[item]));
        ...
        if (!args[2]) {
            switch (cmd){
                ...
                case 'markdown':
                case 'md':
                    let cases = Object.keys(props.cases).map(item => this.printCase(props.cases[item], 'markdown'));
        ...
        switch (cmd) {
            case 'date':
                ...
                container.PubSub.publish('ReleaseScheduled', {room: room});
                break;
            case 'note':
            case 'test':
            case 'step':
    ...
    updateVersion: function(topic, data) {
        const container = this.container, room = data.room;
        let version,
            props = container.getState(room, '', container.config.releaseName);
        if (props) {
            version = props.version ? props.version : '';
    ...
    getReleaseManager: function(room) {
        const container = this.container;
        let releaseManager = container.getState(room, 'releaseManager');
        if (!releaseManager) {
    ...
    getReleaseDate: function(room) {
        const container = this.container;
        let props = container.getState(room, '', container.config.releaseName);
        if (props && props.targetDate) {
            return props.targetDate;
        }
    },

    getReleaseNote: function(room) {
        const container = this.container,
            props = container.getState(room, '', container.config.releaseName);
    ...
    notifyRelease: function(room){
        const release = this.getCurrRelease(room);
        let Notice = "<b>{releaseManager}: Release time reached for {version}!</b> Current status: {status}";
        // use "message" type to bing release manager
        this.container.send(room, Sugar.String.format(Notice, release), 'message');
        let jobKey = 'release-'+room.name;
        ...
        this.container.PubSub.publish('deploy.prod', data);
    },

    getPolicy(room) {
      const container = this.container;
      let policy = container.getState(room, 'policy');
      policy = policy ? policy.split('-') : [];
      /** Example defaultPolicy = ['1D','A'] */
      let defaultPolicy = Object.keys(container.config.releasePolicies)
        .filter(code => container.config.releasePolicies[code].default);

      /** shortPolicy now = { notif: '2 day notification', window: 'Anytime' } filled in with defaults */
      let shortPolicy = [...policy, ...defaultPolicy].reduce((prev, code) => {
        let item = container.config.releasePolicies[code];
        if (!prev[item.policyType]) {
          prev[item.policyType] = item.short;
        }
        return prev;
      }, {});
      return Object.entries(shortPolicy).reduce((result, entry) => {
        result += entry.join(': ') + '<br/>';
        return result;
      }, '');

    },

    checkSchedule(topic, data) {
        const container = this.container;
        let result = container.getQuickStatus(data.room),
          policy = this.shortPolicy(data.room);
        if (topic == 'CheckSchedule') {
            if (result.ready) {
                if (!result.scheduled) {
                    container.send(data.room, `${result.summary} Schedule release. Policy: ${policy}`, 'greenMessage')
                } else {
                    container.send(data.room, `${result.summary} Release already scheduled.`, 'greenMessage')
                }
            }
        } else {
            if (result.ready) {
                container.send(data.room, `${result.summary} Send release notice - !notelify scheduled`, 'greenMessage')
            } else {
                container.send(data.room, `${result.summary} Don't send release notice yet`, 'error')
            }
        }
