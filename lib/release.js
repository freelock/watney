...
module.exports = {
    ...
    setup: function(container) {
        ...
        container.getResult = this.getResult.bind(this);
    ...
    /**
     * Returns icons for: behat, wraith, scheduled.
     * ✔️ ❌
     */
    getResult(room) {
        const container = this.container,
            releaseProps = container.getState(room, '', container.config.releaseName),
            stateProps = container.getState(room, '', container.config.stateName);
        let behat = '❌', wraith = '❌', release = '❌', releaseDate;
        if (stateProps) {
            behat = stateProps.test_behat ? '✔️' : '❌';
            wraith = stateProps.test_wraith ? '✔️' : '❌';
            if (releaseProps.targetDate) {
                releaseDate = new Date(releaseProps.targetDate️).getTime();
                if (releaseDate > Date.now()) {
                    release = '✔️';
                }
            }
        }
        return behat + wraith + release;
    },
