...
module.exports = {
    ...
    remind: function(room, body, event){
        let active, args, currStatus, msg, newState, props, targetDate, jobKey,
        ...
        switch (args[1]) {
            ...
            case 'show':
                ...
                if (state) {
                    ...
                    if (Sugar.Object.size(reminders)) {
                        Sugar.Object.forEach(reminders, function(val) {
            ...
            default:
                ...
                if (result.time && Sugar.Date.isValid(result.time) && result.task) {
                    ...
                    container.send(room, Sugar.String.format('Reminder set for {time}', result));
                } else {
    ...
    setReminder: function(room, time, recipient, message) {
        ...
        let state = container.getState(room, '', container.config.reminderName),
            reminders = {}, keys, jobKey, reminder,
            targetDate, highest = 0, reminderKey;
        if (state) {
            reminders = state; //.getContent();
    ...
    loadScheduledReminders: function() {
        let state, reminders, targetDate, jobKey, container = this.container,
            reminderKey = 1,
    ...
    remindSend: function(room, reminderKey){
        let jobKey = 'reminder-' + room.name + '-' + reminderKey,
        ...
        if (reminder !== undefined) {
            ...
            reminders = Sugar.Object.filter(reminders, function(reminder){
                ...
                let targetDate = Sugar.Date.create(reminder.targetDate);
