            ...
            switch(type) {
              ...
              case 'issue':
                ...
                if (action == 'change') {
                  if (payload.change.comment && payload.change.comment.length) {
                    message += 'Comment on <b><a href="{link}">{item}</a></b> by <b>{by}</b>: ' + payload.change.comment;
                  } else if (payload.change.diff.description) {
                    message += 'Issue <b><a href="{link}">{item}</a></b> description changed by <b>{by}</b>';
                  }
                } else if (action == 'create') {
                  message += 'New issue created by <b>{by}</b>: <a href="{link}">{item}</a>';
                }
