...
module.exports = {
    ...
    async _notelify(data) {
      ...
      const container = this.container, items = {
        ...this.defaultData,
        ...data
      }
      ...
      try {
        if (items.mergeFile) {
          ...
          //fs.close(dataStream);
          if (!fileData) {
            throw new RangeError(`${data.alias} not found in mergeFile.`);
          }
          mergeData = {
    ...
    loadReleaseData(room, data) {
      ...
      data.release.policyHtml = Object.entries(shortPolicy).reduce((result, entry) => {
