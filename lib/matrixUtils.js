...
module.exports = {
    ...
    setRoomList: function() {
        let i, room, state, alias, container = this.container;
...
//                    console.log('loading room for: ', room.roomId, room.name);
        ...
        container.roomList.sort(function(a,b) {
            ...
            let aMsg = a.timeline[a.timeline.length-1];
            if (!aMsg) {
                return -1;
            }
            let bMsg = b.timeline[b.timeline.length-1];
            if (!bMsg) {
                return 1;
            }
    ...
    send: function(room, body, type) {
        ...
        this.container.mx[method](room.roomId, textVersion(body), body);
    },
    ...
    getState: function(room, stateKey, stateName){
        ...
        let currState = room.currentState.getStateEvents(stateName, stateKey);
        if (currState && currState.getContent) {
            let props = currState.getContent();
            if (stateKey.length) {
    ...
    setState: function(room, stateName, data, stateKey, _processing){
        const container = this.container, matrixClient = container.mx, self = this;
        let timeout, next, props;
        ...
        matrixClient.sendStateEvent(room.roomId, stateName, data, stateKey)
           ...
           function(errData){
               if (errData && errData.errcode && errData.errcode === 'M_LIMIT_EXCEEDED') {
                   ...
                   self.stateStack.push([room, stateName, data, stateKey]);
    ...
    sendHelp: function(room, body, event) {
        let cmds = this.container.bangCommands,
            message = "Hi, I'm Watney...<br/>\n" +
            "Your options: <br/><br/>\n\n";
        for (let item in cmds){
            message += "<b>!" + item + " ";
            if (cmds[item].args) {
                message += "["+cmds[item].args.join('|')+"] ";
            }
            message += "</b>- " + cmds[item].help +"<br/>\n";
        }
        this.container.send(room, message, 'notice');
    }
