...
module.exports = {
    ...
    printMaintenance(room, body, event) {
        ...
        const alertRooms = container.filter(room => {
            const roomState = this.loadStatus(room);
            if (!(roomState && roomState.prod_lastrelease && roomState.maintenance)) {
                return false;
            }
            const lastRelease = new Date(roomState.lastRelease),
              elapsed = Date.now() - lastRelease;
            return (roomState.maintenance === 'yes' && elapsed > (1000 * 86400 * 21));
        }, this);
