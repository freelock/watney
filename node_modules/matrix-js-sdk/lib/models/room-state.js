...
RoomState.prototype.setStateEvents = function (stateEvents) {
    ...
    utils.forEach(stateEvents, function (event) {
        ...
        if (event.getType() === "m.room.member") {
            ...
            try {
                for (var _iterator = (0, _getIterator3.default)((0, _keys2.default)(self._sentinels)), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                    var _userId = _step.value;

                    var oldSentinel = self._sentinels[_userId];
                    var newSentinel = new RoomMember(event.getRoomId(), _userId);
                    newSentinel.setMembershipEvent(oldSentinel.events.member, self);
                    newSentinel.setPowerLevelEvent(event);
                    self._sentinels[_userId] = newSentinel;
                }
            } catch (err) {
