 ...
 */
function Crypto(baseApis, eventEmitter, sessionStore, userId, deviceId, clientStore) {
...
Crypto.prototype.getEventSenderDeviceInfo = function (event) {
    ...
    var device = this._deviceList.getDeviceByIdentityKey(event.getSender(), algorithm, sender_key);
...
Crypto.prototype.exportRoomKeys = function () {
    ...
    return q(this._sessionStore.getAllEndToEndInboundGroupSessionKeys().map(function (s) {
        var sess = _this._olmDevice.exportInboundGroupSession(s.senderKey, s.sessionId);
...
Crypto.prototype.importRoomKeys = function (keys) {
    ...
    keys.map(function (session) {
        ...
        var alg = _this2._getRoomDecryptor(session.room_id, session.algorithm);
...
Crypto.prototype._onSyncCompleted = function (syncData) {
    ...
    if (!syncData.oldSyncToken) {
        ...
        if (oldSyncToken) {
            this._invalidateDeviceListsSince(oldSyncToken).catch(function (e) {
                // if that failed, we fall back to invalidating everyone.
                console.warn("Error fetching changed device list", e);
                _this3._invalidateDeviceListForAllActiveUsers();
                return _this3._deviceList.refreshOutdatedDeviceLists();
            }).done();
        } else {
...
Crypto.prototype._invalidateDeviceListsSince = function (oldSyncToken) {
    ...
    return this._baseApis.getKeyChanges(oldSyncToken, this.lastKnownSyncToken).then(function (r) {
        ...
        var filteredUserIds = _this4._getE2eRoomMembers();
        r.changed.forEach(function (u) {
            if (u in filteredUserIds) {
                _this4._deviceList.invalidateUserDeviceList(u);
            }
        });
        return _this4._deviceList.refreshOutdatedDeviceLists();
    });
};
...
Crypto.prototype._invalidateDeviceListForAllActiveUsers = function () {
    ...
    Object.keys(this._getE2eRoomMembers()).forEach(function (m) {
        _this5._deviceList.invalidateUserDeviceList(m);
    });
};
...
Crypto.prototype._getE2eRoomMembers = function () {
    var userIds = Object.create(null);

    var rooms = this._getE2eRooms();
    var _iteratorNormalCompletion2 = true;
    var _didIteratorError2 = false;
    var _iteratorError2 = undefined;

    try {
        for (var _iterator2 = rooms[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
            var r = _step2.value;

            var members = r.getJoinedMembers();
            members.forEach(function (m) {
                userIds[m.userId] = m.userId;
            });
        }
    } catch (err) {
